...
(function() {
    ...
    console.log('Admin API Adapter loading...');
     ...
     */
    const originalApplyTheme = window.applyThemeBtn?.onclick;
    ...
    if (document.getElementById('saveLivestreamBtn')) {
        document.getElementById('saveLivestreamBtn').addEventListener('click', async function(e) {
            ...
            const livestreamConfig = {
                ...
                checkInterval: parseInt(document.getElementById('livestreamCheckInterval').value) * 1000
            };
    ...
    if (document.getElementById('saveGeneralBtn')) {
        document.getElementById('saveGeneralBtn').addEventListener('click', async function(e) {
            ...
            const generalConfig = {
                ...
                slideshowInterval: parseInt(document.getElementById('slideshowInterval').value) * 1000
            };
    ...
    async function loadSettingsFromServer() {
        try {
            const settings = await window.SettingsAPI.getAll();
            console.log('Loaded settings from server:', Object.keys(settings));

            // Apply loaded settings to admin panel
            if (settings.customTheme) {
                console.log('Applying custom theme from server');
            }
            if (settings.customThemes) {
                console.log(`Loaded ${settings.customThemes.length} custom themes from server`);
            }
            if (settings.customSlides) {
                console.log(`Loaded ${settings.customSlides.length} custom slides from server`);
            }
    ...
    console.log('Admin API Adapter loaded successfully');
    console.log('All save operations will now use the centralized API');
